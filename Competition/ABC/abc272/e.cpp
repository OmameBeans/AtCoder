...
int main() {
    ...
    REP(i,N) {
        ...
        r = min((ll)(M+1),(N-A[i]+i) / (i+1));
        for(ll j = l; j < r; j++) {
            if(A[i] + j*(i+1) >= N) continue;
            val[j].push_back(A[i]+j*(i+1));
        }
    }
    ...
    for(int i = 1; i <= M; i++) {
        ...
        // cout << i << " : ";
        // for(auto x : val[i]) cout << x << " ";
        // cout << endl;
        
        vector<bool> ex(val[i].size()+1,false);

        for(auto x : val[i]) if(x < val[i].size()+1) ex[x] = true;
