...
int main() {
    ...
    Child.push_back(1e9+10);
    Adult.push_back(1e9+10);

    int ans = 0;

    for(auto d : Cand) {
        auto aitr = Adult.size() - (lower_bound(Adult.begin(),Adult.end(),d)-Adult.begin())-1;
        auto citr = lower_bound(Child.begin(), Child.end(), d) - Child.begin();

        if(Adult.size() == 1) aitr = 0;
        if(Child.size() == 1) citr = 0;

        //cout << d << " " << aitr << " " << citr << endl;

        chmax(ans,(int)aitr + (int)citr);
    }
