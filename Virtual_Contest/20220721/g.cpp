...
int main() {
    ...
    REP(_,K) {
        ...
        dp[0] = pre[1] + pre[2];
        dp[1] = pre[0]*(W-1) + pre[3] + pre[1]*(W-2);
        dp[2] = pre[0]*(H-1) + pre[3] + pre[2]*(H-2);
        dp[3] = pre[1]*(H-1) + pre[2]*(W-1) + pre[3] * (H+W-4);
    }

    if(x1 == x2) {
        if(y1 == y2) cout << dp[0] << endl;
        else cout << dp[1] / (W-1) << endl;
    } else {
        if(y1 == y2) cout << dp[2] / (H-1) << endl;
        else cout << dp[3] / (H*W - (H-1) - (W-1) - 1) << endl;
    }
    return 0;
}